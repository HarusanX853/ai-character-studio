...
import {
  fallbackIndependentOpinionOutput,
  hostMessageSchema,
...
function parseJsonWithSchema<T>(rawText: string, schema: z.ZodType<T>, fallback: T, normalize?: (candidate: unknown) => unknown) {
  const parsedJson = safeJsonParse(rawText);
  const extractedJson = parsedJson.ok ? parsedJson : extractFirstJsonObject(rawText);
  const candidate = extractedJson.ok ? normalize?.(extractedJson.data) ?? extractedJson.data : fallback;
...
async function buildControlPrompts(params: {
  ...
  const commonContext = [
    ...
    `公开事实: ${stringifyJson(episode.publicFactsJson ?? [])}`,
    ...
    `主持人公开消息: ${stringifyJson(
      episode.hostMessages.map((message) => ({
        kind: message.kind,
    ...
    `公开讨论记录: ${stringifyJson(
      episode.turns.slice(-16).map((turn) => ({
...
export async function runIndependentOpinions(episodeId: string) {
  ...
  for (const participant of episode.characters) {
    ...
    const parsed = parseJsonWithSchema<IndependentOpinionOutput>(
      result.rawText,
      independentOpinionOutputSchema,
      fallbackIndependentOpinionOutput,
      normalizeIndependentOpinionCandidate
    );
    ...
    const opinion = await prisma.$transaction(async (tx) => {
      ...
      await tx.llmCall.create({
        data: {
          ...
          characterId: participant.character.id,
          episodeId: episode.id,
          requestJson: toInputJson(prompts),
...
export async function runGroupDiscussion(episodeId: string, characterId: string) {
  ...
  const parsed = parseJsonWithSchema(
    ...
    normalizeCharacterOutputCandidate
  );
